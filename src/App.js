...
//`https://maps.googleapis.com/maps/api/place/details/json?placeid=${clickedRest.place_id}&key=${process.env.REACT_APP_API_KEY}`
...
export default function App() {
  ...
  function getGoodleReviews(place_id) {
    ...
    function callback(results, status) {
      if (status === window.google.maps.places.PlacesServiceStatus.OK) {
        ...
        setReviews(results.reviews[1].text)
      }
    }
  };
  ...
  const onLoad = React.useCallback(function callback(map) {
    console.log("map from onLoad",map)
          ...
          >
            ...
            >
              ...
              {clickedRest && (
                ...
                >
                  <div className="info" >
                    <h5 className="info-header">{clickedRest.name}</h5>
